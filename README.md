...
A web based static noise machine to create different colors of static noise algorithmically on the fly, and mix custom soundscapes. Static noise is used for sleep, relaxation and focus. It's also used for ADHD and Tinnitus support.
...
- The sound keeps playing when the screen locks.
...
- Long duration mix file exports may fail on mobile.
- Mix file exports are limited to two hours to prevent the browser from crashing due to memory issues.
- Once the site loads the player and the mixer can be used offline. But, when offline, a sound in the sound library is only accessible if it has been used before i.e. the browser has cached it.
- User uploaded tracks should be at least 10 seconds long to ensure that a 3-second crossfade is fully applied. The crossfade ensures there's no clicking sound when the audio loops.
